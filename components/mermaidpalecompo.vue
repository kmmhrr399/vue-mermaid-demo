...
export default {
  ...
  created: function() {
      ...
      database
      .ref(this.id+"/tizu/test/")//thisはuserにかかっている
      .once("value")
      .then(result => {
        if (result.val()) {
  ...
  watch: {
    ...
    DeleteCountLen(){
      this.$nextTick(() => {
        ...
        console.log(this.$store.state.mapData.mapDataList[this.mapDataListLen-1])
        this.deleteCount = this.$store.state.mapData.mapDataList[this.DeleteCountLen-1]
    })
  },
  },
  methods: {
    ...
    unLink(event){
      ...
      const tagetId = parent.id //nextに追加されるのノードのIDの文字列
      const child = this.filterByText(this.nextId)
      alert(child.id.toString()+" 番の"+ child.text+"が親のノードから外されます")
      const node2 = JSON.parse(JSON.stringify(this.data.concat()))
      this.data.length =1
      for (let i = 0; i < node2.length; i++) //該当するID探して、nextに追加する
      {
        if (node2[i].id === tagetId && 'next' in node2) //該当するIDを見つけた時
        {
          ...
          if (node2[i].next.includes(child.id.toString())) {
            alert(node2[i].next.indexOf(child.id.toString()))
